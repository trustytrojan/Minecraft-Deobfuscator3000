...
public class Deobfuscator3000 {
    ...
    public void initialize(Application app, Stage primaryStage) throws Exception {
        ...
        this.mappingsPathBox.setOnDragOver(ev -> {
            if (ev.getDragboard().hasFiles()) {
                ev.acceptTransferModes(new TransferMode[]{TransferMode.LINK});
            }
        });
        this.mappingsPathBox.setOnDragDropped(ev -> {
            this.t1.setText(((File)ev.getDragboard().getFiles().get(0)).getAbsolutePath());
        ...
        this.dir1.setOnAction(ev -> {
            ...
            File f = chooser.showDialog((Window)primaryStage);
            if (f != null) {
        ...
        this.file1.setOnAction(ev -> {
            ...
            chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Mapping files!", new String[]{"*.jar", "*.zip", "*.csv"}));
            chooser.setInitialDirectory(this.actualMapping == null ? FileSystemView.getFileSystemView().getHomeDirectory() : (this.actualMapping.isDirectory() ? this.actualMapping : this.actualMapping.getParentFile()));
            File f = chooser.showOpenDialog((Window)primaryStage);
            if (f != null) {
                this.t1.setText(f.getAbsolutePath());
            }
        });
        ...
        this.modPathBox.setOnDragDropped(ev -> {
            this.t2.setText(((File)ev.getDragboard().getFiles().get(0)).getAbsolutePath());
            ev.setDropCompleted(true);
        });
        ...
        this.file2.setOnAction(ev -> {
            ...
            chooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Java frienly files!", new String[]{"*.java", "*.class", "*.jar", "*.zip"}));
            chooser.setInitialDirectory(this.actualMod == null ? FileSystemView.getFileSystemView().getHomeDirectory() : (this.actualMod.isDirectory() ? this.actualMod : this.actualMod.getParentFile()));
            File f = chooser.showOpenDialog((Window)primaryStage);
            if (f != null) {
                this.t2.setText(f.getAbsolutePath());
        ...
        copyLog.setOnAction(ev -> this.childrensToClipboard((Pane)this.log));
        MenuItem clsLog = new MenuItem("Clear log");
        clsLog.setOnAction(ev -> this.log.getChildren().clear());
        ContextMenu logContextMenu = new ContextMenu(new MenuItem[]{copyLog, clsLog});
        ...
        this.log.addEventFilter(MouseEvent.MOUSE_RELEASED, ev -> {
            if (ev.getButton() == MouseButton.MIDDLE && ev.isControlDown()) {
                ...
                this.childrensToClipboard((Pane)this.log);
            } else if (ev.getButton() == MouseButton.SECONDARY) {
                logContextMenu.show((Node)this.logScoll, ev.getScreenX(), ev.getScreenY());
            } else {
                logContextMenu.hide();
            }
        });
        ...
        this.log.getChildren().addListener((ListChangeListener<Node>) elm -> {
        ...
        this.deobf.disableProperty().bind(Bindings.createBooleanBinding(() -> {
            ...
            return true;
        }, (Observable[])new Observable[]{this.t1.textProperty(), this.t2.textProperty(), this.t1.styleProperty(), this.t2.styleProperty(), this.shouldDeobf.selectedProperty()}));
                ...
                this.deobfThread = new Thread(() -> {
                    try {
                        ...
                        if (f.exists()) {
                            ...
                            VBox.setMargin((Node)open, (Insets)new Insets(7.0));
        ...
        Alert aClose = new Alert(Alert.AlertType.CONFIRMATION, "Are you sure you want to leave?\nIn progress deobfuscation will be canceled!", new ButtonType[]{ButtonType.YES, ButtonType.NO});
    ...
    private String decompile(String sourcePath) {
        ...
        Decompiler.decompile((String)sourcePath, (ITextOutput)new PlainTextOutput((Writer)sw), (DecompilerSettings)DECOMP_SETTINGS);
